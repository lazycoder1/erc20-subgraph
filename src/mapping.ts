...
function loadTransfer(event: Transfer): TransferOb {
  let transfer = TransferOb.load(event.transaction.hash.toHexString() + '-' + event.logIndex.toString());
  if (!transfer) {
    transfer = new TransferOb(event.transaction.hash.toHexString() + '-' + event.logIndex.toString());
  }

  transfer._to = event.params.to.toHexString();
  transfer._from = event.params.from.toHexString();
  transfer._value = event.params.value.toHexString();

  transfer._hash = event.transaction.hash.toHexString();
...
export function handleTransfer(event: Transfer): void {
  let token = loadOrCreateToken(event);
  let transfer = loadTransfer(event);
  if (!token) {
    return;
  }
